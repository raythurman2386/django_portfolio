...
class ServicePageViewTest(TestCase):
    ...
    def test_service_page_view_context(self):
        ...
        self.assertEqual(
            response.context['summary_data'][0].title, "Test Summary")
        self.assertEqual(
            response.context['service_data'][0].title, "Test Service")
        self.assertEqual(
            response.context['contact_data'][0].email, "test@example.com")
